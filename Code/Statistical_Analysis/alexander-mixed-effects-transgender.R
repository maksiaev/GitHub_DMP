...
#Read in the data:
PrimaryModel_data <- read.csv("years_transgender_autism_chatgpt.csv", header = T, stringsAsFactors = T)
...
MNull <- lmer(Score~(1|Year_Label), PrimaryModel_data)

Model_Year <- lmer(Score~Year + (1|Year_Label), PrimaryModel_data)
...
Model_Year_Field <- lmer(Score~Year*Type +(1|Year_Label), PrimaryModel_data)
...
(Emms1a <- emmeans(Model_Year_Field, list(pairwise~Type), adjust="tukey",lmerTest.limit = 16445, pbkrtest.limit = 16445 ))
confint(Emms1a, level = 0.95)


# Test whether adding interaction improves
# 
# Model_Interaction <- lmer(Score~Year + (1|Type), PrimaryModel_data)
# #Model comparison statistics:
# performance::r2(Model_Year_Field)
...
plot_sentiment <- ggplot(PrimaryModel_data, aes(x = Year, y = Score)) +
  geom_point() +
  geom_smooth(method = "lm") +  # Linear regression lines
  labs(y= "Sentiment Score", x= "Year")+
...
plot_sentiment
ggsave("year_field_interactions_autism_transgender_chatgpt.png", width=10, height=8)
...
# 
# plot_sentiment2
# ggsave("year_field_interactions_autism_transgender_chatgpt2.png", width=10, height=8)
# 
# 
